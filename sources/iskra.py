...
_MEETINGS_PAGE_LIMIT = 999   # /meetings now CURSOR-PAGINATES (v1, 2026-06-27; old 500 hard cap lifted).
                             # Page at 999, NOT 1000: at exactly limit=1000 the API returns
                             # has_more=false + NO next_cursor mid-set (a vendor cursor off-by-one),
                             # silently stopping at 1000 -> under-count. 999 paginates correctly to
                             # exhaustion (~6,065 rows / 7 pages). Raise the off-by-one with Arseny.
...
class IskraClient:
    ...
    def meetings(self, since: str | None = None, max_pages: int = 2000) -> PageWalk:
        """GET /v1/meetings — v1 (verified 2026-06-27) now CURSOR-PAGINATES; the old 500 hard cap is
        LIFTED. Page at limit=999 and walk next_cursor to exhaustion (~6,065 rows / 7 pages). Do NOT
        call limit=1000: at exactly 1000 the API returns has_more=false + NO next_cursor mid-set (a
        vendor cursor off-by-one) and silently stops at the first 1000 -> under-count. paginate()
        sets completed=True ONLY when it reached has_more=false, so a truncated walk fails loud (E3).
        `since` IS a working lower bound, but the ingest pulls the COMPLETE set (since=None) because
        the set is small and COMPLETENESS (not incrementality) is the requirement — this is the W1e
        fix for the old 500-newest-only meeting under-count."""
        params = {"limit": _MEETINGS_PAGE_LIMIT}
        if since:
            params["since"] = since
