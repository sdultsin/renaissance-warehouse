...
printf '%s\n%s\n' "$SIG" "$NOW" > "$STATE"
MSG=":rotating_light: Warehouse git DRIFT (box vs origin/main): ${REASONS}| origin=$(git rev-parse --short origin/main) box=$(git rev-parse --short HEAD). Going-forward rule = edit -> PR to origin -> box pulls; the box must not carry unpushed commits or a dirty tree. Runbook: GIT-SYNC-DISCIPLINE.md (repo root)"
