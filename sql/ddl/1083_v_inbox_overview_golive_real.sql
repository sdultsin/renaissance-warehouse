-- 1083_v_inbox_overview_golive_real.sql  [2026-07-07]
-- FIX go_live ("First went live"): it was account_label.cold_start = MIN(date) of ANY send INCLUDING WARM-UP,
-- so ~59k warming MilkBox falsely showed a go-live and the date reflected warm-up, not a real campaign send.
-- Redefine go_live = core.account_first_cold_send.first_cold_send_at (first REAL ue_type=1 "sent from campaign"),
-- shown ONLY for currently Active-tagged inboxes (warm-up-tagged -> NULL = "Not yet"). Active inboxes with no
-- send-log record (~41k) stay NULL (blank/dash) for now. Changes the go_live output column ONLY; the stage CASE
-- still uses l.go_live (unchanged) and every other column is byte-identical to 1047.
-- @gate: alter
-- Depends on 1047, 1082
