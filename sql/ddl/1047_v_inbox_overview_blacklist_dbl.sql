-- 1047_v_inbox_overview_blacklist_dbl.sql  [2026-07-06]
-- FIX the `blacklisted` flag: only count Spamhaus DBL (the deliverability-relevant list), not ANY
-- blocklist. The old bool_or(status='listed') counted SURBL, which lists ~64% of domains (46,021 of
-- 71,733) — almost all false positives; Spamhaus DBL flags only 54. Result: ~232k inboxes were shown
-- "blacklisted" when the real number is a few hundred. Semantics-only; all columns identical to 1046.
-- @gate: alter
-- Depends on 1046
