-- 1046_v_inbox_overview_banned_first.sql  [2026-07-06]
-- FIX stage/health so BANNED wins over DISCONNECTED. The old CASE tested connection_error/
-- sending_error FIRST, so ~4.9k inboxes that are BOTH conn-errored AND warmup-banned were
-- mislabelled 'Disconnected', undercounting Banned (7.3k vs the true ~8.6k). Banned means
-- warmup_status='banned' regardless of connection state; this moves that branch ahead of
-- Disconnected in both `stage` and `health`. Semantics-only; all columns identical to 1045.
-- @gate: alter
-- Depends on 1045
