...
SELECT
    ...
    -- raw_sendivo_campaign_daily is one row per (date, sub, campaign, STATUS_GROUP) -- the
    -- status_group FILTER selects the DELIVERED slice; COALESCE so a group with no delivered
    -- rows reads 0, not NULL (06-26 check: messages 1,183,122 / delivered 1,099,461).
    coalesce(sum(c.n_messages) FILTER (WHERE c.status_group = 'DELIVERED'), 0) AS delivered_messages,
