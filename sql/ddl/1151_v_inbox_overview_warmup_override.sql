-- 1151_v_inbox_overview_warmup_override.sql  [2026-07-21, David]
-- core.v_inbox_overview: warm-up start becomes the OVERRIDE where one exists, and the raw Instantly
-- value is preserved beside it rather than replaced.
--
-- WHY. Suppliers fix a broken mailbox by deleting and re-adding it, which resets timestamp_warmup_start.
-- 35,985 live inboxes already carry a warm-up start LATER than their own first cold send, which is only
-- explicable by a re-add. Instantly stays the default and keeps auto-updating; core.inbox_warmup_override
-- (DDL 1150) holds only the exceptions David has corrected by hand, and where one exists it WINS — here,
-- in the Hub (LIVE_SRC already points at this view, so it inherits this on the next rebuild), and in the
-- Activation Pipeline's flip gate.
--
-- NOTHING IS DESTROYED. warmup_start_instantly keeps the raw census value forever, so an override can
-- always be audited or undone. warmup_start_source says which one you are looking at.
-- warmup_start_previous / recreated_count come from core.inbox_date_history (DDL 1149) — observed
-- transitions, not inference.
--
-- JOIN KEY. Every CTE in this view already keys on lower(trim(email)) and the override loader stores
-- the same form; the new joins do it explicitly so an override cannot silently fail to apply if an
-- upstream source ever stops canonicalising. This is the moderator WARN on DDL 1150, closed here.
--
-- ADDITIVE ONLY: byte-identical to 1085 except the two new CTEs, the warmup_start COALESCE, and four
-- appended columns. Every existing column keeps its name, position and meaning, so the ~100 downstream
-- consumers are untouched.
-- @gate: alter
-- Depends on 1085, 1149, 1150
...
-- The operator's correction, if any. LEFT JOINed, so an EMPTY override table changes nothing at all —
-- that is what makes this safe to ship before a single override exists.
ovr AS (
    SELECT lower(trim(email)) AS email,
           TRY_CAST(warmup_start_override AS TIMESTAMP) AS ws_override
    FROM core.inbox_warmup_override
    WHERE nullif(trim(coalesce(warmup_start_override, '')), '') IS NOT NULL
),
-- Observed history: what the warm-up start used to be, and how many times the mailbox was re-added.
-- recreated_count counts 'created' changes because every warm-up-start reset came with one.
hist AS (
    SELECT lower(trim(email)) AS email,
           max(old_value) FILTER (WHERE field = 'warmup_start') AS ws_previous,
           count(*)       FILTER (WHERE field = 'created')      AS recreated_count
    FROM core.inbox_date_history
    GROUP BY 1
)
SELECT
  ...
  c.created_at,
  COALESCE(ovr.ws_override, c.warmup_start) AS warmup_start,
  c.warmup_start                            AS warmup_start_instantly,
  CASE WHEN ovr.ws_override IS NOT NULL THEN 'override' ELSE 'instantly' END
                                            AS warmup_start_source,
  hist.ws_previous                          AS warmup_start_previous,
  COALESCE(hist.recreated_count, 0)         AS recreated_count, COALESCE(sev.connected_evt, CAST(firstact.first_active_day AS TIMESTAMP)) AS connected_date,
...
LEFT JOIN ovr  USING (email)
LEFT JOIN hist USING (email)
