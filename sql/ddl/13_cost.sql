...
CREATE TABLE IF NOT EXISTS core.cost_ledger (
  ...
  vendor              VARCHAR NOT NULL,       -- 'instantly' | 'dynadot' | 'maxify' | 'tomer' | 'lucas' | 'tucows' | 'folderly' | 'warmly' | 'anthropic' | 'blitzapi' | 'leadmagic' | 'findymail' | 'prospeo' | 'aleads' | 'mv' | 'supabase' | 'vercel' | 'cloudflare' | 'do_droplet' | 'otd' | 'google' | etc.
